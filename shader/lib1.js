...
function start_gl(canvas_id, vertexShader, fragmentShader) {
   setTimeout(function() {
      ...
      function setMouse(event, eventZ) {
         ...
         if (eventZ !== undefined)
        	 gl.cursor.z = eventZ;
         var mixer  = {value: gl.cursor.x + 1, max: 2};
         var filter = {value: gl.cursor.y + 1, max: 2};
         CrossfadeSample.crossfade(mixer);
         CrossfadeSample.changeFrequency(filter);
      }
...
function gl_init(gl, vertexShader, fragmentShader) {
   ...
   for(var i = 0; i < side; i++){
  	 var r = 0.98*Math.sin(5*2*Math.PI/side*i + Math.PI/12) + 0.02*Math.sin(23*2*Math.PI/side*i);
  	 arr.push(r*r*Math.cos(2*Math.PI/side*i));
  	 arr.push(r*r*Math.sin(2*Math.PI/side*i));
  	 arr.push(0);
  	 if(i%2 == 1){
  		 arr.push(0);
  		 arr.push(0);
...
function gl_update(gl) {
   ...
   gl.drawArrays(gl.TRIANGLE_STRIP, 0, 1501);                            // Render the square.
